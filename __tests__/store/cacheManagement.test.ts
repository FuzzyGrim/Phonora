...
describe('Music Player Store - Cache Management', () => {
  ...
  describe('clearCache', () => {
    ...
    it('should handle directory deletion failure and delete files individually', async () => {
      ...
      mockFileSystem.deleteAsync
        ...
        .mockResolvedValue();
      mockFileSystem.readDirectoryAsync.mockResolvedValueOnce(['song1.mp3', 'cover1.jpg']);
    ...
    it('should handle errors gracefully', async () => {
      mockFileSystem.getInfoAsync.mockRejectedValueOnce(new Error('Filesystem error'));
  ...
  describe('isFileCached', () => {
    ...
    it('should handle errors and return false', async () => {
      mockFileSystem.getInfoAsync.mockRejectedValueOnce(new Error('File access error'));
  ...
  describe('getCachedFilePath', () => {
    ...
    it('should handle different file extensions', () => {
      ...
      expect(store.getCachedFilePath('cover1', 'jpg')).toBe(CACHE_DIRECTORY + 'cover1.jpg');
      expect(store.getCachedFilePath('song1', 'flac')).toBe(CACHE_DIRECTORY + 'song1.flac');
    });
  });
  ...
  describe('getCacheSize', () => {
    ...
    it('should handle errors and return 0', async () => {
      mockFileSystem.getInfoAsync.mockRejectedValueOnce(new Error('Access error'));
  ...
  describe('hasEnoughCacheSpace', () => {
    ...
    it('should handle errors and return false', async () => {
      ...
      const getCacheSizeSpy = jest.fn().mockRejectedValue(new Error('Access error'));
  ...
  describe('getCachedFiles', () => {
    ...
    it('should skip files that cannot be accessed', async () => {
      mockFileSystem.getInfoAsync.mockResolvedValueOnce({ exists: true } as any);
      mockFileSystem.readDirectoryAsync.mockResolvedValueOnce(['song1.mp3', 'corrupt.mp3']);
      mockFileSystem.getInfoAsync
        .mockResolvedValueOnce({ exists: true, size: 1024, modificationTime: 1000 } as any)
  ...
  describe('freeUpCacheSpace', () => {
    it('should delete oldest files to free up required space', async () => {
      ...
      // Mock getCachedFiles
      const originalGetCachedFiles = useMusicPlayerStore.getState().getCachedFiles;
      ...
      expect(mockFileSystem.deleteAsync).toHaveBeenCalledWith(CACHE_DIRECTORY + 'old.mp3');
      expect(mockFileSystem.deleteAsync).toHaveBeenCalledWith(CACHE_DIRECTORY + 'newer.mp3');
    ...
    it('should stop deleting when enough space is freed', async () => {
      ...
      expect(mockFileSystem.deleteAsync).toHaveBeenCalledWith(CACHE_DIRECTORY + 'file1.mp3');
  ...
  describe('downloadSong', () => {
    ...
    it('should download and cache song when caching is enabled', async () => {
      ...
      const originalHasEnoughCacheSpace = useMusicPlayerStore.getState().hasEnoughCacheSpace;
      const hasEnoughCacheSpaceSpy = jest.fn().mockResolvedValue(true);
      useMusicPlayerStore.setState({ hasEnoughCacheSpace: hasEnoughCacheSpaceSpy });

      mockFileSystem.getInfoAsync.mockResolvedValueOnce({ exists: true } as any);
      mockFileSystem.downloadAsync.mockResolvedValueOnce({ status: 200 } as any);
    ...
    it('should free up space when cache is full', async () => {
      ...
      const originalHasEnoughCacheSpace = useMusicPlayerStore.getState().hasEnoughCacheSpace;
      const hasEnoughCacheSpaceSpy = jest.fn()
        .mockResolvedValueOnce(false) // First check: not enough space
        .mockResolvedValueOnce(true);  // After cleanup: enough space
      useMusicPlayerStore.setState({ hasEnoughCacheSpace: hasEnoughCacheSpaceSpy });

      const originalFreeUpCacheSpace = useMusicPlayerStore.getState().freeUpCacheSpace;
    ...
    it('should handle download errors', async () => {
      ...
      mockFileSystem.getInfoAsync.mockResolvedValueOnce({ exists: true } as any);
      mockFileSystem.downloadAsync.mockResolvedValueOnce({ status: 404 } as any);

      const store = useMusicPlayerStore.getState();
      
      await expect(store.downloadSong(mockSong)).rejects.toThrow('HTTP Error: 404');
  ...
  describe('downloadImage', () => {
    ...
    it('should handle download errors', async () => {
      ...
      await expect(store.downloadImage('cover1', 'Test Song')).rejects.toThrow('HTTP Error: 404');
