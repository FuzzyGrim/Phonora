...
describe('Music Player Store - Authentication', () => {
  ...
  describe('initializeStore', () => {
    it('should load credentials from secure storage and set authenticated state', async () => {
      ...
      mockSecureStore.getItemAsync.mockResolvedValue(JSON.stringify(mockConfig));
      mockAsyncStorage.getItem.mockResolvedValue(JSON.stringify(mockUserSettings));
    ...
    it('should handle credentials without user settings', async () => {
      ...
      expect(currentState.config).toEqual(mockConfig);
      expect(currentState.isAuthenticated).toBe(true);
      expect(currentState.userSettings).toEqual({ offlineMode: false, maxCacheSize: 10 });
      expect(mockFetchSongs).toHaveBeenCalled();
    });
    ...
    it('should handle storage errors gracefully', async () => {
      ...
      expect(currentState.config).toBeNull();
      expect(currentState.isAuthenticated).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith('Error initializing store:', expect.any(Error));
  ...
  describe('setConfig', () => {
    ...
    it('should handle secure storage errors gracefully', async () => {
      ...
      expect(currentState.config).toBeNull();
      expect(currentState.isAuthenticated).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith('Error saving credentials:', expect.any(Error));
  ...
  describe('clearConfig', () => {
    it('should remove credentials from secure storage and reset state', async () => {
      ...
      useMusicPlayerStore.setState({
        ...
        songs: [{ id: '1', title: 'Test Song', artist: 'Test Artist', album: 'Test Album', duration: 180 }],
      });
    ...
    it('should handle storage deletion errors gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error clearing credentials:', expect.any(Error));
  ...
  describe('getCoverArtUrl', () => {
    it('should generate correct cover art URL when config exists', () => {
      ...
      expect(coverArtUrl).toContain('https://demo.subsonic.org/rest/getCoverArt.view');
